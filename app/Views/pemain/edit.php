...
<body class="dashboard-body">
    ...
    <div class="sidebar text-center">
        ...
        <a href="javascript:void(0);" onclick="logout()" class="fas-fa-sign-out-alt">
    ...
    <div class="content">
        ...
        <div class="container mt-4">
            ...
            <form action="/pemain/update/<?= esc($pemain['id_mlbb']) ?>" method="post" class="mt-3">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label for="id_mlbb" class="form-label">ID MLBB:</label>
                        <input type="text" id="id_mlbb" name="id_mlbb" value="<?= esc($pemain['id_mlbb']) ?>"
                            class="form-control" required maxlength="255">
                    </div>

                    <div class="col-md-6 mb-3">
                        <label for="nickname" class="form-label">Nickname:</label>
                        <input type="text" id="nickname" name="nickname" value="<?= esc($pemain['nickname']) ?>"
                            class="form-control" required>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label for="nama" class="form-label">Nama:</label>
                        <input type="text" id="nama" name="nama" value="<?= esc($pemain['nama']) ?>"
                            class="form-control" required>
                    </div>
                        ...
                        <select id="id_role" name="id_role" class="form-select" required>
                            ...
                            <?php foreach ($roles as $role): ?>
                                <option value="<?= esc($role['id_role']) ?>"
                                    <?= $pemain['id_role'] == $role['id_role'] ? 'selected' : '' ?>>
                                    <?= esc($role['role']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" id="rank" name="rank" value="<?= esc($pemain['rank']) ?>"
                            class="form-control" required>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label for="winrate" class="form-label">Winrate (%):</label>
                        <input type="number" step="0.01" id="winrate" name="winrate"
                            value="<?= esc($pemain['winrate'] ?? '') ?>" class="form-control" required>
                    </div>

                    <div class="col-md-12 mb-3">
                        <label for="kompetisi_menang" class="form-label">Kompetisi yang Pernah Dimenangkan:</label>
                        <input type="text" id="kompetisi_menang" name="kompetisi_menang"
                            value="<?= esc($pemain['kompetisi_menang'] ?? '') ?>" class="form-control" required>
                    </div>
                </div>
